...
impl BirdeyeClient {
    ...
    fn get_token_address(symbol: &str) -> AgentResult<&'static str> {
        let symbol = symbol.trim_start_matches('$').to_uppercase();
        TOKEN_ADDRESSES
            .iter()
            .find(|(s, _)| *s == symbol)
            .map(|(_, addr)| *addr)
            .ok_or_else(|| AgentError::InvalidInput(format!("Unknown token symbol: {}", symbol)))
    }
    ...
    async fn get_market_data(&self, address: &str) -> AgentResult<TokenMarketResponse> {
        self.rate_limit().await;

        let url = format!(
            "{}/v2/tokens/token_data?address={}",
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| AgentError::ApiError(format!("Birdeye request failed: {}", e)))?;

        let status = response.status();
        let text = response
            .text()
            .await
            .map_err(|e| AgentError::ApiError(format!("Failed to read Birdeye response: {}", e)))?;
...
impl BirdeyeApi for BirdeyeClient {
    ...
    async fn get_token_info_by_address(&self, address: &str) -> AgentResult<TokenInfo> {
        ...
        Ok(TokenInfo {
            ...
            trade_24h: 0,   // Not available in this endpoint
        })
    }
}
