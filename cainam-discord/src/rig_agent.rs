...
impl RigAgent {
    pub async fn new() -> Result<Self> {
        ...
        let embeddings = EmbeddingsBuilder::new(embedding_model.clone())
            .add_document(Document::new("Rig_guide", md1_content))
            .add_document(Document::new("Rig_faq", md2_content))
            .add_document(Document::new("Rig_examples", md3_content))
            .build()
            .await?;

        vector_store.add_embeddings(embeddings).await?;
